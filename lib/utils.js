...
function mergeBlockData(currentState, newBlock) {
    ...
    const updatedBlock = { // Initialize the updated block (copy)
        ...
        units: new Array(numValues).fill(undefined)
    };
    ...
    if (updatedBlock.units && oldUnits) { // Copy old units
        for (let i = 0; i < 4; i++) {
    ...
    if (updatedBlock.units && newBlock.units) { // Merge the new units (only analog)
        for (let i = 0; i < 4; i++) {
